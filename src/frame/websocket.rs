...
impl Frame for WebSocketFrame {
    fn new<T: Any>(buf: &[u8], args: &Vec<T>) -> WebSocketFrame {
        let arg_1 = &args[0] as &Any;
        let arg_2 = &args[1] as &Any;

        let frame_type = arg_1.downcast_ref::<FrameType>().unwrap().clone();
        let op_type = arg_2.downcast_ref::<OpType>().unwrap().clone();
